...
#    common_functions.create_index (fc, ['TileId', 'ClumpId'], 'TileClumpIdx')  
#    common_functions.log_mp (log_file, 'Ready to process ' + str(len(clumped_tile_pairs)) + ' clumps')
#
#    in_mem_fc = 'in_memory\\' + arcpy.Describe(fc).baseName
#    arcpy.CopyFeatures_management(fc, in_mem_fc)
#
#    i=0
...
#            with arcpy.da.UpdateCursor(in_mem_fc, attr_list, query, sr, False) as cursor:
#                ####################################################################### 
...
#            del cursor
#            
#    arcpy.CopyFeatures_management(in_mem_fc, fc)
#    arcpy.Delete_management(in_mem_fc)
#    common_functions.log_mp (log_file, 'Worker process ending')
#    return;
...
# This function gets called on a new process, processes it's assigned set of clumps and writes
# the results out to a feature class where the parent process can find it
#
# Input tuple is <input feature class, clumped tile pair list, logfile>
def process_clumps_mp (tuple):
    # Unpack the input tuple
    fc_input, fc_output, clumped_tile_pairs, min_clump_id, max_clump_id, log_file = tuple
    ...
    common_functions.log_mp(log_file, 'Populating output feature class')        
...
def prepare_mp_tuples (fc_input, clumped_tile_pairs,temporary_assets):
    ...
    for i in range(_threads):
        ...
        mp_tuples.append((fc_input, fc_output, clumped_tile_pairs_mp, min_clump_id, max_clump_id, common_functions.log_mp_fn))
...
def merge (fc_input, fc_output):
    ...
    try:              
        ...
        for tuple in mp_tuples:
            fc_input, fc_output_mp, clumped_tile_pairs, min_clump_id, max_clump_id, log_file = tuple
